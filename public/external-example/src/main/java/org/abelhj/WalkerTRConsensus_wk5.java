...
public class WalkerTRConsensus_wk5 extends ReadWalker<Integer,Integer>  {
	...
	try {
		...
		char strand=spl[5].charAt(0); 
		Amplicon amp=new Amplicon(chr, start, end, spl[3], strand);
	...
	for (String chr: ampliconMap.keySet()) {
	    for (Integer end : ampliconMap.get(chr).keySet()) {
		for(Amplicon amp: ampliconMap.get(chr).get(end)) {
