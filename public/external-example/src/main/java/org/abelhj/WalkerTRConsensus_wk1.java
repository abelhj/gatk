...
public class WalkerTRConsensus_wk1 extends ReadWalker<Integer,Integer>  {
    ...
    Map<String, Map<String, ReadFamily >  bcreads;
	...
	try {
		...
		if(!ampliconMap.get(chr).containsKey(end)) {
		    ampliconMap.get(chr).put(end, new List<Amplicon> ());
		}
	...
	bcreads=new LinkedHashMap<String,  Map<String, ReadFamilyAmp > > bcreads();
	...
	if(ref!=null) {
	    ...
	    if(  !(curchr.equals(oldchr)) || ampliconMap.get(curchr).contains(curpos-10)) {         //find last possible current amplicon
		for(Integer endpos : ampliconMap.get(oldchr).keySet()) {
		    if (chrpos>endpos+10 || !(curchr.equals(oldchr))) {
			for(Amplicon amp : ampliconMap.get(oldchr).get(endpos)) {
			    Map<String, ReadFamily> perAmpBC=bcreads.get(amp.getName());
