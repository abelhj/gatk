...
public class HomRun1 extends ReadWalker<Integer,Integer>  {
    ...
    int maxNM=99;
    @Argument(fullName = "runpos", shortName = "runpos", doc="position overlapped by homopolymer run", required=true) 
    int runpos=31022442;
	...
	if(ref!=null) {
		...
		if(read.getFirstOfPairFlag() && read.getIntegerAttribute("NM")<maxNM && read.getAlignmentStart()<runpos && read.getAlignmentEnd()>runpos ) {
		    int readpos=ReadUtils.getReadCoordinateForReferenceCoordinate(read, runpos, ReadUtils.ClippingTail.LEFT_TAIL);
    ...
    public void onTraversalDone(Integer result) {
		...
		System.out.print(len+"\t"+counts[len]+"\n");
	    }
    }
